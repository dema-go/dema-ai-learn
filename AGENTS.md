...
- 用户点选答案后立即提交并判题，不再增加单独的“确认答案”按钮。
- 正确选项原位显示绿色边框与勾；选错时用户选择显示红色边框与叉，同时标出绿色正确答案。
- 解释紧跟选项展示，原文依据按需展开；“下一题 / 查看结果”固定在底部安全区上方。
- 答题选项接近屏幕全宽，同组选项使用 `16px` 间距；普通同组按钮使用 `12px`，主行动组与次行动组使用 `20px`。
- 答案提交响应中的 `chosen_index` 是服务端已持久化选择。丢失响应后重试时，前端必须用该字段恢复服务端事实，不得保留本次重试点击的不同选项。

微信基础库会注入高优先级的
`wx-button:not([size=mini]) { width: 184px; margin-left: auto; margin-right: auto; }`。
仅在 `.btn` 或 `.choice-card` 上写 `width: 100%` 仍可能被覆盖。当前实现使用
`.screen .btn` 和 `.choice-list .choice-card` 提高类选择器优先级并显式清除左右自动外边距。
修改按钮布局时必须在微信开发者工具的 WXML 计算样式和模拟器截图中确认真实盒模型，不能只依赖静态 WXSS 测试。
...
- 答题响应始终返回服务端持久化的 `chosen_index`，前端用它处理丢失响应后的幂等重试；
- `attempt(user_id, quiz_id)` 与 `answer(attempt_id, question_id)` 由唯一索引保护，并通过版本化轻量迁移执行重复数据预检；检测到历史重复数据时启动失败并要求备份后人工处理，不静默删除业务记录；
